...
public class UHCJoinModEvent implements Listener
{
    ...
    public void event(JoinModEvent event)
    {
        ...
        Bukkit.getPlayer(event.getPlayer()).setScoreboard(UHC.getPlugin().getArenaTeam().getScoreboard());
        Bukkit.getPlayer(event.getPlayer()).getInventory().clear();
        
        for(ArenaPlayer player : UHC.getPlugin().getArenaTeam().getArenaPlayers())
        {
